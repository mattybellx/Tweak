...
function getFilesInPath(source) {
  ...
  function getDirectories(source2) {
    return readdirSync(source2).map((name2) => {
      ...
      if (_windows$f) {
        try {
          util$g.powerShell(
            ...
            if (!error) {
              let lines = stdout.toString().split("\r\n");
              const description2 = util$g.getValue(lines, "description", ":");
              const version2 = util$g.getValue(lines, "SMBIOSBIOSVersion", ":");
              if (description2.indexOf(" Version ") !== -1) {
                result2.vendor = description2.split(" Version ")[0].trim();
                result2.version = description2.split(" Version ")[1].trim();
              } else if (description2.indexOf(" Ver: ") !== -1) {
                result2.vendor = util$g.getValue(lines, "manufacturer", ":");
                result2.version = description2.split(" Ver: ")[1].trim();
              } else {
                result2.vendor = util$g.getValue(lines, "manufacturer", ":");
        ...
        if (_windows$e) {
          try {
            ...
            Promise.all(workload).then((data) => {
              ...
              if (name2.indexOf("@") >= 0) {
                result2.brand = name2.split("@")[0].trim();
                result2.speed = name2.split("@")[1] ? parseFloat(name2.split("@")[1].trim()) : 0;
                _cpu_speed = result2.speed;
              } else {
                result2.brand = name2.trim();
                result2.speed = 0;
              }
              result2 = cpuBrandManufacturer(result2);
      ...
      if (_darwin$c) {
        exec$c("system_profiler SPMemoryDataType", (error, stdout) => {
          ...
          if (!result2.length) {
            ...
            const type2 = util$e.getValue(lines, "      Type:");
          ...
          util$b.promiseAll(workload).then((data) => {
            ...
            devices.forEach((device) => {
              ...
              if (size) {
                ...
                if (i === -1 || serialNum === "") {
                  i = util$b.findObjectByKey(result2, "name", name2);
                }
                if (i !== -1) {
        ...
        if (JSON.stringify(ifaces) === JSON.stringify(_ifaces) && !rescan) {
          ...
          for (let dev in ifaces) {
            ...
            echo -n "wirelessspeed: "; iw dev ${ifaceSanitized} link 2>&1 | grep bitrate; echo;`;
              ...
              type2 = operstate === "up" ? util$a.getValue(lines, "wireless").trim() ? "wireless" : "wired" : "unknown";
  ...
  try {
    ...
    try {
      ...
      parts.forEach((ifaceDetails) => {
        ...
        const type2 = util$9.getValue(lines, "GENERAL.TYPE");
        const id = i++;
        ...
        util$9.powerShell(cmd).then((stdout) => {
          ...
          parts.forEach((part) => {
            ...
            if (lines.length >= 5) {
              ...
              const type2 = util$9.getValue(lines, "Radio type", ":", true) || util$9.getValue(lines, "Type de radio", ":", true) || util$9.getValue(lines, "Funktyp", ":", true) || null;
          ...
          try {
            util$8.powerShell("Get-CimInstance Win32_Process | select ProcessId,Caption,UserModeTime,KernelModeTime,WorkingSetSize | ConvertTo-Json -compress").then((stdout, error) => {
              if (!error) {
                ...
                if (processesString !== "*") {
                  let processesMissing = processes2.filter((name2) => procStats.filter((item) => item.name.toLowerCase().indexOf(name2) >= 0).length === 0);
            ...
            if (stdout) {
              ...
              lines.forEach((line) => {
                ...
                if (data.length > 4) {
                  ...
                  const name2 = _linux$6 ? linuxName : data[4].substring(data[4].lastIndexOf("/") + 1);
                  procStats.push({
                    name: name2,
              ...
              if (processesString !== "*") {
                let processesMissing = processes2.filter((name2) => {
...
function getLinuxUsbType(type2, name2) {
  let result2 = type2;
...
function getDarwinUsbType(name2) {
  ...
  if (name2.indexOf("camera") >= 0) {
    ...
    result2 = "Mouse";
  } else if (name2.indexOf("microp") >= 0) {
    result2 = "Microphone";
...
const readConfig = () => {
  ...
  const configPath = path$4.join(basePath, "ai.config.json");
  const raw = fs$b.readFileSync(configPath, "utf-8");
  return JSON.parse(raw);
};
const downloadModel = async (url, onProgress) => {
  const config = readConfig();
  const targetDir = path$4.dirname(config.modelPath);
  fs$b.mkdirSync(targetDir, { recursive: true });
  ...
  return new Promise((resolve) => {
    ...
    const request = https.get(downloadUrl, (res) => {
      ...
      const fileStream = fs$b.createWriteStream(config.modelPath);
      res.on("data", (chunk) => {
...
const runLlama = (config, prompt) => {
  return new Promise((resolve, reject) => {
    const threads = Math.max(4, Math.floor(os$a.cpus().length / 2));
    const args = [
      "--model",
      config.modelPath,
      "--prompt",
    ...
    node_child_process.execFile(config.binaryPath, args, { timeout: 12e4, windowsHide: true }, (error, stdout) => {
      if (error) {
        reject(error);
        return;
      }
...
const analyzeWithLocalAi = async (input) => {
  const config = readConfig();
  const modelExists = fs$b.existsSync(config.modelPath);
  const binaryExists = fs$b.existsSync(config.binaryPath);
...
const createWindow = () => {
  const win = new electron.BrowserWindow({
    ...
    webPreferences: {
      preload: path$4.join(__dirname, "../preload/preload.js")
    }
  });
  mainWindow = win;
  ...
  if (devUrl) {
    ...
    win.loadFile(path$4.join(__dirname, "../renderer/index.html"));
  }
...
const emitLog = (message) => {
  const entry = `[${(/* @__PURE__ */ new Date()).toLocaleTimeString()}] ${message}`;
...
const openLogWindow = () => {
  ...
  logWindow.loadFile(path$4.join(__dirname, "../renderer/log.html"));
  logWindow.on("closed", () => {
    logWindow = null;
  });
};
const buildIssues = (snapshot) => {
  const issues = [];
...
const getSystemSnapshot = async () => {
  const [osInfo2, cpu2, mem2, load, disks] = await Promise.all([
    si.osInfo(),
    si.cpu(),
    si.mem(),
    si.currentLoad(),
  ...
  const memoryUsedPercent = (mem2.total - mem2.available) / mem2.total * 100;
  ...
  return {
    os: `${osInfo2.distro} ${osInfo2.release}`,
    cpuModel: `${cpu2.manufacturer} ${cpu2.brand}`,
    memoryTotalGb: Number((mem2.total / 1024 ** 3).toFixed(1)),
    memoryUsedPercent: Number(memoryUsedPercent.toFixed(1)),
    cpuLoad: Math.max(0, Math.min(100, cpuLoad)),
    disks: diskInfo,
    uptimeHours: Number((osInfo2.uptime / 3600).toFixed(1))
  };
};
electron.app.whenReady().then(() => {
  createWindow();
  electron.ipcMain.handle("system:getSnapshot", async () => getSystemSnapshot());
  electron.ipcMain.handle("system:getIssues", async () => {
    const snapshot = await getSystemSnapshot();
    return buildIssues(snapshot);
  });
  electron.ipcMain.handle("system:getAiAssessment", async () => {
    const snapshot = await getSystemSnapshot();
    const issues = buildIssues(snapshot);
    return analyzeWithLocalAi({ snapshot, issues });
  });
  electron.ipcMain.handle("system:downloadModel", async (_event, url) => {
    const config = readConfig();
    const result2 = await downloadModel(url, (progress) => {
      mainWindow?.webContents.send("system:downloadProgress", progress);
    });
    return { ...result2, modelPath: config.modelPath };
  });
  electron.ipcMain.handle("log:open", () => {
    openLogWindow();
  });
  electron.ipcMain.on("log:write", (_event, message) => {
    emitLog(message);
  });
